...
def train_knn_model():
    ...
    try:
        ...
        all_country_ids = [country.Id for country in db.session.query(Countries.Id).all()]
...
def predict_knn(user_id, n_neighbors=10):
    ...
    favorite_movie_ids = (
        session.query(FavoriteList.MovieId)
        .filter(FavoriteList.UserId == user_id)
        .all()
    )
...
def train_svd_model():
    ...
    training_data = [(rating.UserId, rating.MovieId, rating.Rating) for rating in ratings_query]
    df = pd.DataFrame(training_data, columns=['UserId', 'MovieId', 'Rating'])

    # Tạo một Reader để đọc dữ liệu từ DataFrame
    reader = Reader(rating_scale=(1, 5))  # Điều chỉnh theo phạm vi rating của bạn (1 đến 5)
...
def predict_rating(user_id, list_movieId):
    ...
    print(sorted_arr)
    res = []
    for id,est in sorted_arr:
        movie =  db.session.query(Movies, func.round(func.avg(Rating.Rating), 1).label("AverageRating")).join(
                    Rating, Movies.Id == Rating.MovieId
                ).filter(Movies.Id == id).group_by(Movies.Id).first()
